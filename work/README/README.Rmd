...
```{r}
test_pred <- ft_predict(model, new_data=test, k = 1L)
str(test_pred)

confusion_matrix <- table(as.integer(labels), as.integer(gsub("\\D", "", test_pred$label)))
confusion_matrix
...
### Train Model
```{r}
cntrl <- ft_control(learning_rate = 0.025, word_vec_size = 5, epoch = 1, 
                    nthreads = 10L, verbose = 2L)

model <- ft_train("fil9", method = "skipgram", control = cntrl)
model

ft_save(model, "fil9_skipgram_model.bin")
```

### Load Model
```{r}
model <- ft_load("fil9_skipgram_model.bin")
```
